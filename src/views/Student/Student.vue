    ...
    <base-header class="pb-6">
      <div class="row align-items-center py-4">
        <div class="col-lg-6 col-7">
          <h6 class="h2 text-white d-inline-block mb-0">Students Data</h6>
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <route-bread-crumb></route-bread-crumb>
          </nav>
        </div>
        <div class="col-lg-6 col-5 text-right">
          <base-button size="sm" @click="filterModal = true;" type="neutral">Filter</base-button>
                ...
                <template>                    
                    <form role="form">
                       ...
                       <base-input label="Class:">                    
                        ...
                        <div >
                            <base-button @click="fillGrid()" :disabled="saving" type="primary" class="col" ><i v-bind:class="signF"></i> {{ optionF }} </base-button>
                        </div>
                    </form>
                </template>
            </card>
         ...
         <modal :show.sync="filterCols"
            ...
            <card type="secondary" shadow
                ...
                <template>                    
                    <form role="form">                    
                      <div class="row">
                             <base-checkbox v-for="item in cols" :key="item.prop" v-model="item.checked" class="ml-3 mb-3 mr-1" style="width:130px;">
                               {{ item.label }} 
                            </base-checkbox>
                      </div>
                        ...
                        <div >
                            <base-button @click="SaveCols()" :disabled="savingCols" type="primary" class="col mt-3" ><i v-bind:class="signC"></i> {{ optionCols }} </base-button>
                        </div>
                    </form>
                </template>
            </card>
          ...
          <div>
            ...
            >
              ...
              <div style="margin-bottom:20px; margin-top:-10px;">
                  <base-button style="margin-top:5px;" icon type="primary" @click="Add()">
                        <span class="btn-inner--icon"><i class="fas fa-plus"></i></span>
                        <span class="btn-inner--text">Add new student</span>            
                </base-button>

                  <base-button style="margin-top:5px;" type="warning" @click="Register()">
                        <span class="btn-inner--icon"><i class="fas fa-plus"></i></span>
                        <span class="btn-inner--text">Register new student</span>            
                </base-button>
  ...
  data() {
    ...
    return {
      ...
      cmbStatus:{
          ...
          data: [ {id: '-', name: 'All'}, {id: 'Present', name: 'Present'}, {id: 'Registered', name: 'Registered'}, {id: 'Online', name: 'Online'}, {id: 'Left', name: 'Left'}]
      }, 
      cmbClass:{
          value: {id: "-", name: 'All'},
          data: []
      ...
      cmbGender:{
          value: {id: "-", name: 'All'},
          data: [{id: "-", name: 'All'}, {id: 'Male', name: 'Male'}, {id: 'Female', name: 'Female'}]
      },
      cols: null,
  ...
  methods: {
      ...
      Add(){
            ...
            let resolved = this.$router.resolve('/fusion/admission');
              ...
              {
                let routeData = this.$router.resolve({name: resolved.route.name, query: {} });
      ...
      fillDropdowns(){
          ...
          //Classes
          var url = this.$store.state.domain + '/ClassandSectionInfo?cID=' + this.$store.state.cID +'&_db=' + this.$store.state.db          
           axios.get(url)
          .then((response) => {
          ...
          .catch(function (error) {
           ...
           //Faculty
          var url = this.$store.state.domain + '/Faculty?cID=' + this.$store.state.cID +'&_db=' + this.$store.state.db          
           axios.get(url)
          .then((response) => {
            ...
            response.data.data.forEach(element => {                         
               this.cmbFaculty.data.push( { id: element.id, name: element.faculty });            
      ...
      {        
          ...
          var url = this.$store.state.domain + '/Fill/Sections?cID=' + this.$store.state.cID +'&_db=' + this.$store.state.db  + "&clsID=" + option.id        
           axios.get(url)
          .then((response) => {
      ...
      fillGrid(){
          ...
          this.saving = true;
           var url = this.$store.state.domain + '/Registration?cID=' + this.$store.state.cID + '&_db=' + this.$store.state.db + '&status=' + this.cmbStatus.value.name + "&cls=" + this.cmbClass.value.name + "&sec=" + this.cmbSec.value.name + "&faculty=" + this.cmbFaculty.value.name + "&gender=" + this.cmbGender.value.name + "&pg=" + this.pIndex;      
    ...
    handleDelete(index, row) {
      swal({
        ...
        if (result.value) {
          ...
          //Remove API
           var url = this.$store.state.domain + '/Batch/Delete?id=' + row.id + '&_db=' + this.$store.state.db
           //console.log.log('Remove: ' + url);   
           axios.post(url, qs.stringify(""))
          .then((response) => {           
            if(response.data.status === "error")
            {
              this.$notify({verticalAlign: 'bottom', horizontalAlign: 'center', message: 'Request Error! <br />' + response.data.data, type: 'danger', icon: 'fa fa-ban', timeout: 0 });
              
            }
            else{
             this.$notify({verticalAlign: 'bottom', horizontalAlign: 'center', message: 'Saved! <br />Batch "' + row.batch + '" removed.', type: 'danger', icon: 'fa fa-check', timeout: this.$store.state.timeout });                      
