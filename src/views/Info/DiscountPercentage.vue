    ...
    <base-header class="pb-6">
      <div class="row align-items-center py-4">
        <div class="col-lg-6 col-7">
          <h6 class="h2 text-white d-inline-block mb-0">Fee Status</h6>
            ...
            >
              <div style="margin-bottom: 5px">
                <base-button icon type="primary" @click="AddModal()">
                  ...
                  ></span>
                  <span class="btn-inner--text">Add Fee Status</span>
                </base-button>
...
export default {
  ...
  methods: {
    Validate() {
      if (this.myObj.name.length === 0) {
        this.nameError = "Enter Fee Status";
      } else if (this.myObj.FeesStatus.length === 0) {
        this.dscperError = "Enter Fee Status";
      } else if (this.myObj.FeesStatus > 100) {
        this.dscperError = "Fee Status cannot be greater than 100";
      } else if (this.myObj.FeesStatus < 0) {
        this.dscperError = "Fee Status cannot be less than 0";
      } else {
        //Save to DB
            ...
            .then((response) => {
              if (response.data.status === "error") {
                ...
                this.$notify({
                  ...
                  horizontalAlign: "center",
                  message:
                    'Saved! <br />Fee Status "' + this.myObj.name + '" added.',
                  type: "success",
                  icon: "fa fa-check",
                  timeout: this.$store.state.timeout,
                });
    ...
    editRecord(index, row) {
      ...
      this.myObj.FeesStatus = row.FeesStatus;
    ...
    handleDelete(index, row) {
      swal({
        title: "Are you sure?",
        text: "You want to delete Fee Status " + row.name + "?",
        type: "warning",
            ...
            .then((response) => {
              if (response.data.status === "error") {
                ...
                this.$notify({
                  ...
                  horizontalAlign: "center",
                  message:
                    'Saved! <br />Fee Status "' + row.name + '" removed.',
                  type: "danger",
                  icon: "fa fa-check",
                  timeout: this.$store.state.timeout,
                });
